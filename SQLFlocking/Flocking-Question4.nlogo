...
globals [filename dataquestion4]
...
to setup
  ...
  create-turtles population
    [ set size 1.5  ;; easier to see
      ifelse random-float 1 < proportion-males
        [set sex "male" set speed male-speed]
        [set sex "female" set speed female-speed]
      setxy random-xcor random-ycor 
      ifelse sex = "male"
        [set color blue]
        [set color pink]
    ]
    create-table ;; TABLE TO BE EXPORTED TO SQL
  reset-ticks
end


to create-table
  ; configuring a default connection to the default connection to the sql database
  sql:configure "defaultconnection" [["host" "localhost"] ["port" 3306]
    ["user" "root"] ["password" "root"] ["database" "flocking_db"] ["autodisconnect" "on"]]
  ;; standard commands in SQL. By convention, commands are capitalized. 
  sql:exec-direct (word "DROP TABLE IF EXISTS " table-name)
  sql:exec-direct (word "CREATE TABLE IF NOT EXISTS " table-name " (vision FLOAT, avg_deviation_of_flockmates FLOAT)")
  ;; names of the fields that we are going to include in our database. 
end

to go
  ifelse ticks < sim.ticks [ ;; iterations
  ask turtles [ flock ]
  ;; the following line is used to make the turtles
  ;; animate more smoothly.
  repeat 5 [ask turtles [fd speed / 25] display]

  if ticks > 100 [set-plot-x-range ticks - 100 ticks]
  set-current-plot-pen "total"
  plotxy ticks mean [deviation] of turtles
  set-current-plot-pen "males"
  plotxy ticks mean [deviation] of turtles with [sex = "male"]
  set-current-plot-pen "females"
  plotxy ticks mean [deviation] of turtles with [sex = "female"]
  if export-data = true [write-to-file]
  tick
  ]
  [export-db stop] ;; EXPORT DATABASE TO SQL
end

to export-db ;; will be my "insert-agents" in MySQL.nlogo
  sql:configure "defaultconnection" [["host" "localhost"] ["port" 3306]
    ["user" "root"] ["password" "root"] ["database" "flocking_db"] ["autodisconnect" "on"]]
    ;; one line of sql for each of my agents. each of my turtles will say:
    sql:exec-direct (word "INSERT INTO " table-name "(vision, avg_deviation_of_flockmates) VALUES (" vision " ," mean [deviation] of turtles ")")
end

to question4
    sql:configure "defaultconnection" [["host" "localhost"] ["port" 3306]
    ["user" "root"] ["password" "root"] ["database" "flocking_db"] ["autodisconnect" "on"]]
    sql:exec-query "SELECT `avg_deviation_of_flockmates` FROM flockingresults" []
    set dataquestion4 sql:fetch-resultset 
    (r:put "dataquestion4" dataquestion4)
    (r:eval "data<-as.data.frame(dataquestion4)")
    (r:eval "data<-t(data)")
    (r:eval "dimnames(data)<-c()")
    (r:eval "plotquestion4<-boxplot(data[,2]~data[,1], xlab='vision', ylab='average deviation')")
    let graph (r:get "plotquestion4")
  print dataquestion4
  print graph
end
...
to flock  ;; turtle procedure
  ...
  ifelse any? flockmates
    [set deviation abs (heading - average-flockmate-heading)]
    [set deviation 0]
end
...
to choose-file
  ...
  file-open filename
  file-type (word "Tick, Avg Deviation of All Turtles, Avg Deviation of Males, Avg Deviation of Females")
  file-print ""
  file-close
end
...
to write-to-file
  file-open filename
  file-type (word ticks ", " mean [deviation] of turtles ", ")
  file-type (word mean [deviation] of turtles with [sex = "male"] ", ")
  file-type (word mean [deviation] of turtles with [sex = "female"])
  file-print ""
  file-close
end
...
195
485
mean dev from flockmates
mean [deviation] of turtles
2
1
11
