...
;; Create males
  ...
  let i initial-males
  foreach sort-on [dominance] of birds with [sex = males] [
...
;; Create females
 create-turtles initial-females
    [ set sex females
      set shape "bird side"
      set size 0.3
      set color orange
      ...
      ifelse ((count trees-here / (count birds-here)) >= female-min-resources) and any? birds-here with [sex] males
        [set happy? true]
        [set happy? false]
    ]
  
;; Assign males to be happy or unhappy (according to the their minimum resources threshold)
  foreach sort-on [rank] birds with [sex] males [ 
    ask ? [
      ifelse (count trees-here / (count birds-here)) >= male-min-resources
      [set happy? true]
      [set happy? false]
    ]
  ]
    ...
    ask patches [
      set number-males count birds-here with [sex] males
      set number-females count birds-here with [sex] females
